...
description: "ペットの健康記録や通院履歴を管理するWebアプリ。認証・画像保存・Azure運用設計・E2Eテストまで含めて開発しています。"
...
import PetHealthAppMockup from '../../components/PetHealthAppMockup.astro';
...
このプロジェクトでは画面機能だけでなく、認証・画像保存・自動テスト・Azureへのデプロイ・機密情報管理・障害時の復旧までを一つの開発範囲として扱っています。要件定義書・基本設計書・ER図・テストケースは[リポジトリのdocs/](https://github.com/Cat5Dog2/PetHealthManagement/tree/main/docs)で公開しています。
...
- 入力値と環境設定を検証し、Productionでの不正な設定を起動時に検知
...
- **Playwright E2E**: 個人開発では手動テストに限界があるため導入。CIに組み込むことでリグレッションを防止
...
[公開アプリ](https://app-pethealth-prod-dkedakd8c0g2hza2.japanwest-01.azurewebsites.net/)で新規登録すると、作成したアカウントでログインして主要機能を確認できます。
